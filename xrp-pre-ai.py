...
def add_log(msg):
    ...
    if len(log_messages) > 4:  # keep last 4 for screen space
        log_messages.pop(0)
...
def get_radar_report():
    if radar_buffer:
        info = radar.parse_radar_report()
        if info:
            display.text(f"x={info[0]}, y={info[1]}, s={info[3]}, r={info[4]}", 5, 103, 1)
            display.show()
            return(True)
        else:
            return(False)
...
def run_program(index):
    ...
    if index == 0: safety_drive(0.6, 0.6, 15.0)
        ...
        time.sleep(0.5)
        safety_drive(-0.7, -0.7, -8.0)
    # No more index 8 or 9 here (now handled as live modes)
    elif index == 11:  # FOLLOW
        follower.run()
    elif index == 12:  # GO 5M
        avoider.run()
...
def set_distance_mode():
    ...
    while True:
        ...
        draw_distance_bar(get_radar_instance())
        display.show()
...
def main():
    ...
    while True:
        ...
        if imu_mode or radar_mode:
            bar_y = 72          # higher when active
            data_y = 88         # data start
            log_y = data_y + 30 # log below data (room for 3 lines)
        else:
            bar_y = 88          # normal
            data_y = 0          # no data
            log_y = bar_y + 15  # log right below bar
